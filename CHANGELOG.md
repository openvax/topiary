...
and categorical columns. The public `EvalContext.df` view uses those same keys,
so third-party DSL nodes remain aligned with the context's group index.
Canonical counts are validated before duplicate rows are collapsed, so booleans
and malformed values cannot disappear based on row order or an incomplete
sample. Exact large numeric values and Arrow-backed columns are supported up to
the canonical nullable-Int64 limit; larger individual or pooled counts raise a
