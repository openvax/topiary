...
| `predict_from_antigens(fragments)` | `[AntigenFragment]` | Universal path — any origin; fragment-level metadata and `target_intervals` threaded through. |
| `predict_from_variants(variants)` | VariantCollection | Variant pipeline (builds `AntigenFragment`s internally and delegates). |
