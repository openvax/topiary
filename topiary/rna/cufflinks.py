...
def load_cufflinks_gene_fpkm_dict(fpkm_filename, remap_novel_genes_onto_ensembl_ids):
    """
    Load FPKM expression values for Ensembl genes.

    Parameters
    ----------
    fpkm_filename : str
        Cufflinks tracking file with FPKM values for gene IDs

    remap_novel_genes_onto_ensembl_ids : bool
        Should novel gene IDs have their expression value reassigned to
        overlapping Ensembl genes?
    """
    if remap_novel_genes_onto_ensembl_ids:
        # if the FPKM file we've been given was generated by Cufflinks in
        # "novel" transcript mode, then we may end up with transcripts
        # that are perfectly contained within a known gene but still have
        # a non-Ensembl identifier. In this case the user can use the
        # --rna-remap-novel-genes-onto-ensembl flag to
        # approximate Ensembl gene expression levels by adding up
        # novel genes that fall within the boundaries of known Ensembl
        # genes.
        gene_expression_df = load_cufflinks_dataframe(fpkm_filename)
        return remap_novel_gene_expression_onto_ensembl_ids(gene_expression_df)
    else:
        return load_cufflinks_fpkm_dict(fpkm_filename)