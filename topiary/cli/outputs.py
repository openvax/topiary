...
def add_output_args(arg_parser):
    output_group = arg_parser.add_argument_group(
        title="Output", description="How and where to write results"
    )

    output_group.add_argument(
        "--output-csv", default=None, help="Path to output CSV file"
    )
...
def write_outputs(
    ...
    if args.output_csv:
        ...
        df.to_csv(args.output_csv, index=True, index_label="#", sep=args.output_csv_sep)
